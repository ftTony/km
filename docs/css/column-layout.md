...
- 三个部分都设定为浮动，然后设置center的高度为100%，此时，left和right部分会跳到下一行；
...
#### 4.1 利用正padding+负margin

我们通过等布局便可以解决圣杯布局的第二点缺点，因为背景是在padding区域显示的，**设置一个大数值的padding-bottom，并在所有列外面加上一个容器，并设置 overflow:hidden 把溢出背景切掉。**这种可能实现多列等高布局，并且也能实现列与列之间分隔线效果，结构简单，兼容所有浏览器。新增代码如下：
